...
export function CountdownTimer({ checkInDateStr, checkOutDateStr, actualCheckInAt, ratePlanKind, tzOffset = "+08:00" }: CountdownTimerProps) {
  ...
  useEffect(() => {
    ...
    if (!ratePlanKind || ratePlanKind === "24h") {
      if (checkOutDateStr) {
        ...
        expirationDate = new Date(`${checkOutDateStr.slice(0, 10)}T12:00:00${tzOffset}`);
      }
    } else {
      ...
      if (actualCheckInAt) {
        ...
        const hoursToAdd = parseInt(ratePlanKind.replace(/\D/g, ""), 10) || 0;
        expirationDate = new Date(`${checkInDateStr.slice(0, 10)}T14:00:00${tzOffset}`); // Assuming 2PM standard check-in as fallback
        expirationDate.setHours(expirationDate.getHours() + hoursToAdd);
      }
    }
