...
export function ReceiptModal({ booking, onClose }: ReceiptModalProps) {
  ...
  const {
    ...
    ...booking,
    restaurant_charges_total: restaurantTotal,
  });
            ...
            <div className="bg-slate-50 p-4 rounded-xl print:bg-transparent print:border print:border-black print:p-2">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between border-b border-slate-200 pb-1">
                  ...
                  <span className="font-medium text-slate-900">{booking.rooms?.room_number ?? "N/A"} <span className="text-slate-500 font-normal">({booking.rooms?.room_type ?? "N/A"})</span></span>
                </div>
                <div className="flex justify-between border-b border-slate-200 pb-1">
                  <span className="text-slate-500">Rate Plan</span>
                  <span className="font-medium text-slate-900 uppercase">{booking.rate_plan_kind ?? "N/A"}</span>
                </div>
                <div className="flex justify-between border-b border-slate-200 pb-1">
                  <span className="text-slate-500">Check In</span>
                  <span className="font-medium text-slate-900">{booking.check_in_date ?? "N/A"}</span>
                </div>
                <div className="flex justify-between pb-1">
                  <span className="text-slate-500">Check Out</span>
                  <span className="font-medium text-slate-900">{booking.check_out_date ?? "N/A"}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {extensionsTotal > 0 && (
                  <tr>
                    <td className="py-4 pr-4">
                      <div className="font-semibold text-slate-800">Stay Extensions</div>
                      <div className="text-[11px] text-slate-500 mt-0.5">Additional stay charges from approved extensions</div>
                    </td>
                    <td className="py-4 text-center">--</td>
                    <td className="py-4 text-right font-medium text-slate-900">PHP {extensionsTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                  </tr>
                )}
                {earlyCheckInFee > 0 && (
                  <tr>
                    ...
                    <td className="py-4 text-center">1</td>
                    <td className="py-4 text-right font-medium text-slate-900">PHP {earlyCheckInFee.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                  </tr>
                )}
                {lateCheckOutFee > 0 && (
                  <tr>
          ...
          <div className="flex justify-end pb-8">
            <div className="w-full sm:w-2/3 md:w-1/2">
              <div className="flex justify-between mb-2 text-sm">
                ...
                <span className="text-slate-900 font-medium">PHP {grandTotal.toFixed(2)}</span>
              </div>
              <div className="flex justify-between mb-4 text-sm pb-4 border-b border-slate-300">
                ...
                <span className="text-emerald-600 font-medium print:text-black">- PHP {deposit.toFixed(2)}</span>
              </div>
              <div className="flex justify-between mb-4 text-sm">
                <span className="font-bold text-slate-600">Total Paid</span>
                <span className="text-emerald-700 font-bold print:text-black">PHP {totalPaid.toFixed(2)}</span>
              </div>
              <div className="flex flex-row justify-between items-center bg-slate-100 p-4 rounded-lg print:bg-transparent print:border-2 print:border-black print:p-2 ml-auto w-full gap-4">
                ...
                <span className="text-xl sm:text-2xl font-bold text-[#07008A] print:text-black truncate">PHP {balance.toFixed(2)}</span>
              </div>
            </div>
          </div>
