...
export default function AdminUsersPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOpen(false); setEmail(""); setPassword(""); setRoleId("3"); setIsActive(true);
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Add admin user</DialogTitle></DialogHeader>
              <form className="space-y-4" onSubmit={handleSave}>
                ...
                <div className="space-y-2"><Label htmlFor="admin-password-u">Password</Label><Input id="admin-password-u" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="At least 8 characters" required /></div>
        ...
        <CardContent className="p-0">
          {loading ? <div className="p-6 space-y-4">{[1, 2, 3, 4, 5].map((i) => <Skeleton key={i} className="h-12 w-full" />)}</div> : (
            <div className="overflow-x-auto">
              <table className="w-full"><thead><tr className="border-b bg-slate-50/80"><th className="text-left py-3 px-6 text-xs font-semibold text-slate-600 uppercase tracking-wider">Email</th><th className="text-left py-3 px-6 text-xs font-semibold text-slate-600 uppercase tracking-wider">Role</th><th className="text-left py-3 px-6 text-xs font-semibold text-slate-600 uppercase tracking-wider">Active</th></tr></thead>
                <tbody>{users.map((u) => <tr key={u.id} className="border-b last:border-0 hover:bg-slate-50/50 transition-colors"><td className="py-4 px-6 font-medium text-[#07008A]">{u.email ?? "—"}</td><td className="py-4 px-6 text-sm text-slate-600">{u.role_id != null ? ROLE_LABELS[u.role_id] ?? u.role_id : "—"}</td><td className="py-4 px-6"><Badge variant={u.is_active ? "default" : "secondary"}>{u.is_active ? "Yes" : "No"}</Badge></td></tr>)}</tbody>
              </table>
            </div>
          )}
        </CardContent>
