  ...
  return (
    <>
      ...
      <AdminConfirmDialog
        ...
        intent="warning"
        onConfirm={async () => { const b = checkOutBooking; if (!b?.id) return; try { const tzOffset = localStorage.getItem("app_timezone_offset") || "+08:00"; const actual = checkOutAt ? new Date(`${checkOutAt}${tzOffset}`).toISOString() : new Date().toISOString(); const res = await api(`/api/bookings/${b.id}/check-out`, { method: "POST", body: JSON.stringify({ actual_check_out_at: actual }) }); const data = await res.json().catch(() => ({})); if (!res.ok) { toast.error(getErrorMessage(data) || "Check-out failed."); return; } const fee = Number((data as { late_checkout_fee_applied?: number }).late_checkout_fee_applied || 0); const roomStatusWarning = typeof (data as { room_status_warning?: unknown }).room_status_warning === "string" ? (data as { room_status_warning: string }).room_status_warning : ""; if (roomStatusWarning) { toast.warning(roomStatusWarning, { duration: 8000 }); } else { toast.success(fee > 0 ? `Checked out. Late fee: ₱${fee.toFixed(2)}.` : "Checked out."); } setCheckOutBooking(null); refreshData(); } catch { toast.error("Something went wrong."); } }}
      >
        <div className="space-y-2">
