...
- The shift cash report workbook is now template version `2`: it renames the extras band to `EXTRAS`, inserts a compact `SCHEDULE` column after `GUEST NAME`, truncates workbook `Ref No.` display to the last 4 digits, and adds a clear top border across the `TOTAL` row.
- Shift cash report activity rows now carry `scheduled_check_in_at`, `scheduled_check_out_at`, and `remaining_balance_due`; the workbook only shows schedule text for unpaid reservation rows that are not yet checked in.
